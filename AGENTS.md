...
当前状态：九大类（数据结构 / 排序 / 图算法 / 动态规划 / 回溯与搜索 / 字符串 / 数学与数论 / 计算几何 / 查找）各有 92 个中英学习条目；中文/英文 Docs 侧栏均为 10 组、94 个非 Home 入口；`src/algorithms` 下有 77 个 `*.module.ts`。C131 `/en` 95 页目录与 190 页静态产物已双轨上线；2026-07-28 本地 300 个 L3/L4 测试文件 / 2138 个用例全绿，104 个 L5 文件 / 118 个用例、coverage 与双 base 全绿。C134 已 verified：主域提供 account meta、ads.txt、真实首页与中英文信任页面，算法站仅在 production/selfhost 注入 AdSense loader；双轨已验证。C127 为 in-progress/97%，T1/T2/T3-A/T3-B/T3-C/T3-D1-B/T3-D2-B/T3-D3-C/T3-D4-A/T3-D4-B/T3-D4-C/T4 已完成，下一步 T5。C133 已 verified：公开 `IllegalCreed/marketing-ops` 使用七工具 MCP v3 contract，所有调用要求 `projectId`；本地 `algorithm-visualizer` Project Profile 驱动仓库、canonical origins、渠道和 DEV tags，secret/runtime state 仅在本机，幂等键、receipt、activation 与 campaign policy 均按项目隔离。GitHub、Bluesky、DEV、Mastodon 已完成 Owner 授权的真实闭环；Bluesky/Mastodon 临时记录已清理，DEV 正式文章长期公开但当前 key 为 reauth-required，后续写入前需重新隐藏 setup；微博 Free 零写额度，adapter disabled。T4 已补齐确定性 1h/48h/7d 跟进计划、标准化跨渠道报告、FAQ-only GitHub Issue 固定模板与 Bug Issue 最小化分流；所有真实回复/建 Issue 仍要求 matching Owner 授权、已知 published receipt、fresh reread 与可用 transport。Owner 硬约束为零新增费用、无企业主体、不泄露凭据；Reddit 后备，微信/B站/X 禁用，微博保留人工分享和后续 RPA 评审。C125 已由 C129 撤销，当前不加载行为分析 tracker，只保留 UTM；C134 AdSense 为独立广告能力。执行事实源是 `docs/marketing/execution-backlog.md`；C134/C133/C127/C131/C132 分别见对应 `docs/plans/` 目录。

C127 T4 插件事实：在既有 Mastodon 段落修复和自包含 MCP bundle 上，新增三窗口计划、标准报告、project-scoped policy、GitHub Issue FAQ 回复与 Bug Issue 双重幂等；51 文件 / 252 用例、coverage、verify、安装缓存 STDIO 与泄漏扫描全绿。当前安装版为 `0.1.0+codex.20260728143703`。
