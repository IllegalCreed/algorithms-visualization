...
- `pnpm test:unit` —— Vitest（jsdom + `@vue/test-utils`）监听模式；单次运行用 `pnpm test:unit:run` 或加 `run`（`pnpm test:unit run <file>`），按名称过滤 `-t "<name>"`，覆盖率用 `pnpm coverage`。2026-07-27 本地现状：300 个测试文件 / 2137 个 L3/L4 用例全绿。
- `pnpm exec playwright test [<name>]` —— L5 端到端（真机 Chromium），用例在 `e2e/*.e2e.ts`（2026-07-27 本地 104 个文件 / 118 个用例全绿）。
...
当前状态：九大类（数据结构 / 排序 / 图算法 / 动态规划 / 回溯与搜索 / 字符串 / 数学与数论 / 计算几何 / 查找）各有 92 个中英学习条目；中文/英文 Docs 侧栏均为 10 组、94 个非 Home 入口，首组是复杂度与学习路径工具；`src/algorithms` 下有 77 个 `*.module.ts`；播放器可插拔轨约 20 条。C131 `/en` 95 页目录与 190 页静态产物已双轨上线；C132 已补齐中文侧栏工具组；2026-07-27 本地 300 个 L3/L4 测试文件 / 2137 个用例全绿，104 个 L5 文件 / 118 个用例、coverage 与双 base 全绿。C134 已 verified：`illegalscreed.cn` 提供 account meta、ads.txt、真实首页与中英文信任页面但不加载广告脚本；`algo.illegalscreed.cn` 仅在 production/selfhost 注入 AdSense meta/loader，并提供 ads.txt 与双语隐私入口；双自托管、算法站 Pages 和线上代表 URL 已验证。C127 为 in-progress/94%，T1/T2/T3-A/T3-B/T3-C/T3-D1-B/T3-D2-B/T3-D3-C/T3-D4-A/T3-D4-B/T3-D4-C 已完成，下一步 T4。C133 已 verified：公开仓库使用七工具 MCP v3 contract，所有调用要求 `projectId`；本机 personal plugin 位于 `~/plugins/marketing-ops`，已安装/enabled，源码公开在 `IllegalCreed/marketing-ops`，secret 与 runtime state 仍仅保存在本机；本地 `algorithm-visualizer` Project Profile 驱动仓库、canonical origins、渠道和 DEV tags，幂等键、receipt 与 GitHub activation 按项目隔离。GitHub 和 Bluesky 的 Owner 授权 smoke 均已完成，临时记录已清理且保持 ready/enabled；微博 Free 零写额度，API adapter disabled。DEV 固定 Forem v1、`dev-article@0.2.0`、Keychain/0600 activation、隐藏 setup、惰性 runtime 与 metrics/comments collector 已完成；Owner 授权的 `marketing-ops-t3d3-smoke-127` 于 2026-07-15 完成 publish、完整正文对拍、同 receipt 幂等复放、feedback 与 `1h` report 读取。receipt `4146005` 为 published，正式文章长期公开；2026-07-27 只读复查显示现有 DEV API key 为 reauth-required，后续 DEV campaign 前需重新隐藏 setup。Mastodon 旧 token 已于 2026-07-27 通过官方 regenerate 失效，替代 token 经本机隐藏 PTY 写入 Keychain；本地 `acct` 补全和 CLI 退出缺陷由 plugin `bb62731` 修复，44 文件 / 225 用例、coverage、verify、validator 与泄漏扫描全绿，单通道只读复查为 ready/enabled。Owner 授权的 T3-D4-C 临时 status 已完成正文读取、同 payload 幂等复放、feedback、1h report 与 delete；段落还原回归修复后 receipt 为 deleted，无缓存官方 API 为 404，账号状态列表为空。C125 已由 C129 撤销，当前不加载行为分析 tracker、不发送站内事件，只保留 UTM；C134 AdSense 为独立广告能力。Header 搜索入口紧跟站点标题，语言与外链保留右侧。Owner 硬约束为零新增费用、无企业主体，宣传首期 API 只做 GitHub、Bluesky、DEV、Mastodon，Reddit 后备，微信/B站/X 禁用；微博保留人工分享和后续 RPA 评审。首次接入由 Codex 带向导，日常只需自然语言 campaign。执行事实源是 `docs/marketing/execution-backlog.md`；C134 事实源是 `docs/plans/20260727-c134-adsense-readiness/`；C133 事实源是 `docs/plans/20260727-c133-multi-project-marketing-ops/`；C131/C132 分别见 `docs/plans/20260711-c131-en-full-parity/` 与 `docs/plans/20260711-c132-zh-learning-tools-menu/`；C-034 与 C125 均已 superseded。

C127 T3-D4-C 最终插件事实：`44a7e9a` 修复 Mastodon HTML 段落还原与未知结果恢复；`7cc60a5` 将 MCP server 改为自包含 bundle 并增加无 `node_modules` 隔离 STDIO。当前安装版 `0.1.0+codex.20260727115555` 已从 Codex 缓存完成七工具握手。
