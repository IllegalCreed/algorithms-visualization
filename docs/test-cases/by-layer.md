...
| docs | TC-DOC-AUTO-127-04           | C127 in-progress/55%、下一步 T3 与记忆一致       | `docs/plans/20260711-c127-auto-distribution/test-cases.md` |
...
| L3   | TC-AUTO-SETUP-127-01..03     | 五渠道安全接入、隐藏录入与脱敏诊断               | personal plugin `src/setup/setup.spec.ts`                  |
| L3   | TC-AUTO-SECRET-127-01..02    | Keychain stdin 边界与 REAUTH_REQUIRED            | personal plugin `src/security/keychain.spec.ts`            |
| L3   | TC-AUTO-PROFILE-127-01..02   | 0700 Profile 隔离与 challenge 失败关闭           | personal plugin `src/security/profile.spec.ts`             |
| L3   | TC-AUTO-QUEUE-127-01..02     | campaign 并发串行化与异常释放                    | personal plugin `src/runtime/campaign-lock.spec.ts`        |
| L3   | TC-AUTO-RECEIPT-127-01..02   | 幂等、公开字段、0600 原子存储与损坏拒绝          | personal plugin `src/storage/receipt-store.spec.ts`        |
| MCP  | TC-AUTO-TRANSPORT-127-01..02 | stdio-only 配置与七工具真实 client smoke         | personal plugin `.mcp.json` + `scripts/stdio-smoke.mjs`    |
