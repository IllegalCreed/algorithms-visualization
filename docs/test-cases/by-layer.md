...
> 2026-07-09 增量说明：本次补录 C-20260709-119 / C-20260709-121 / C-20260709-122 新增维护用例；2026-07-10 为 C-119 补录完整拼音映射与多音字用例，登记 C-123 六个增长执行文档 Case、C-124 十九个 SEO/GEO Case 与 C-129 三个回滚边界 Case；2026-07-11 登记 C-126 二十三个多语言 Case、C-127 六个渠道自动化设计 Case、十九个 T1、二十个 T2、二十个 T3-A、十六个 T3-B 与二十二个 T3-C Case、C130 三十七个英文扩容 Case、C131 四十六个全量英文对齐 Case与 C132 五个中文菜单回归。2026-07-27 登记 C133 十三个多项目通用化 Case与 C134 十四个 AdSense Case。历史分层总数未做全量重算。

## 2026-08-10 移动播放器、数组画布与 CSP 收口（C-20260810-143）

当前实现已 verified/100%；两条 L5 几何用例按先红后绿通过，304/2178 单测、coverage、136/136 E2E、双 base 构建与自有域/Nginx ops 冒烟全绿。

| 层级       | Case ID          | 标题                         | 自动化路径                     |
| ---------- | ---------------- | ---------------------------- | ------------------------------ |
| L5         | TC-PLAYER-143-01 | 移动倍率与计数双列重心对齐   | `e2e/responsive.mobile.e2e.ts` |
| L5         | TC-VIZ-143-02    | 数组短内容完整、满载内部横滚 | `e2e/responsive.mobile.e2e.ts` |
| ops/manual | TC-OPS-143-03    | 自托管安全头与控制台冒烟     | manual + deployment log        |

## 2026-08-10 固定 Header 层叠修复（C-20260810-141）

| 层级 | Case ID          | 标题                        | 自动化路径                     |
| ---- | ---------------- | --------------------------- | ------------------------------ |
| L5   | TC-HEADER-141-01 | 滚动内容不能覆盖固定 Header | `e2e/responsive.mobile.e2e.ts` |

## 2026-08-10 播放器控件与可视化轨道窄屏修复（C-20260810-142）

当前实现为 verified/100%；L5 三条几何用例、build 资源地址门禁、全量浏览器与双 base 产物均通过。提交 `de8da04`、`3ef748a`，Pages run `31358485357`。

| 层级  | Case ID          | 标题                                | 自动化路径                     |
| ----- | ---------------- | ----------------------------------- | ------------------------------ |
| L5    | TC-PLAYER-142-01 | 播放器控件保持方形且不被 grid 拉伸  | `e2e/responsive.mobile.e2e.ts` |
| L5    | TC-VIZ-142-02    | 空队列完整显示、非空车道可内部滚动  | `e2e/responsive.mobile.e2e.ts` |
| L5    | TC-VIZ-142-03    | 桶与计数轨保持单行并可内部横滚      | `e2e/responsive.mobile.e2e.ts` |
| build | TC-BUILD-142-04  | 预渲染静态资源不残留 preview origin | `scripts/verify-seo.mjs`       |
...
| 层级     | Case ID              | 标题                                               | 自动化路径                                           |
| -------- | -------------------- | -------------------------------------------------- | ---------------------------------------------------- |
| L5       | TC-RESPONSIVE-140-01 | 手机首页/Header Sheet 与无横向溢出                 | `e2e/responsive.mobile.e2e.ts`                       |
| L4/L5    | TC-RESPONSIVE-140-02 | Docs 抽屉、焦点守卫与完整正文宽度                  | Docs hooks spec、`e2e/responsive.mobile.e2e.ts`      |
| L4/L5    | TC-RESPONSIVE-140-03 | 手机播放器单列、检查区 tab 与 44px 控件            | player specs、`e2e/responsive.mobile.e2e.ts`         |
| L4/L5    | TC-RESPONSIVE-140-04 | 文档路由滚动重置与标题焦点                         | Docs hooks spec、`e2e/responsive.mobile.e2e.ts`      |
| L4/L5    | TC-RESPONSIVE-140-05 | 交互目标与修饰键退出播放器全局快捷键               | AlgorithmPlayer spec、`e2e/playback-controls.e2e.ts` |
| L4/L5    | TC-RESPONSIVE-140-06 | 搜索 dialog 互斥、焦点循环/恢复与 Enter 守卫       | SearchPalette spec、search/mobile e2e                |
| L3/L4    | TC-RESPONSIVE-140-07 | 结构输入、live region 与键盘等价操作               | `src/components/structures/accessibility.spec.ts`    |
| build    | TC-RESPONSIVE-140-08 | 英文拆包、modulepreload、入口 gzip 与 CSS 输出预算 | `scripts/verify-bundle.mjs`                          |
| L3/build | TC-RESPONSIVE-140-09 | 默认 child、catch-all 与目录式静态入口             | `src/router/index.spec.ts`、SEO/prerender scripts    |

## 2026-08-09 算法播放器阴影裁剪修复（C-20260809-138）

| 层级 | Case ID                     | 标题                                    | 自动化路径            |
| ---- | --------------------------- | --------------------------------------- | --------------------- |
| L5   | TC-PLAYER-SHADOW-138-01..02 | 桌面阴影完整性与窄屏单列/无横向溢出回归 | `e2e/dijkstra.e2e.ts` |

## 2026-08-09 算法播放器四面板网格与防重叠（C-20260809-139）

| 层级 | Case ID                   | 标题                                    | 自动化路径                                                                 |
| ---- | ------------------------- | --------------------------------------- | -------------------------------------------------------------------------- |
| L4   | TC-PLAYER-GRID-139-01..02 | 四面板结构、柱轨父宽约束与箭头槽位回归  | `src/components/player/AlgorithmPlayer.spec.ts`、BarsView/ArrowTrack specs |
| L5   | TC-PLAYER-GRID-139-03..04 | 1440px 不重叠、900px 单列顺序与无横溢出 | `e2e/binary-answer.e2e.ts`                                                 |

## 2026-08-09 算法播放器同屏布局（C-20260809-137）

| 层级 | Case ID                     | 标题                             | 自动化路径                       |
| ---- | --------------------------- | -------------------------------- | -------------------------------- |
| L4   | TC-PLAYER-LAYOUT-137-01..03 | 舞台分区、步骤同步与代码行滚动   | `src/components/player/*spec.ts` |
| L5   | TC-PLAYER-LAYOUT-137-04..05 | 桌面双栏、900px 单列与无横向溢出 | `e2e/dijkstra.e2e.ts`            |

## 2026-07-27 AdSense 主域审核与算法站接入（C-20260727-134）

| 层级   | Case ID                 | 标题                                                  | 自动化路径                             |
| ------ | ----------------------- | ----------------------------------------------------- | -------------------------------------- |
| script | TC-ADS-ROOT-134-01..04  | 主域静态授权、所有权、真实内容、信任页与示例清理      | personal `pnpm adsense:check`          |
| L3     | TC-ADS-ALGO-134-01..03  | publisher 常量、build-only head 与预渲染请求隔离      | `src/monetization/adsense.spec.ts`     |
| L4     | TC-ADS-ALGO-134-04      | Footer 中英文隐私链接与外链属性                       | `src/views/Home/Footer/Footer.spec.ts` |
| build  | TC-ADS-BUILD-134-01..03 | production/selfhost 190 页与 development 零注入       | verify/selfhost/local smoke            |
| L5     | TC-ADS-LIVE-134-01..03  | 两域静态资源、代表 HTML、注入次数和 HTTP 状态线上复查 | online HTTP/HTML assertions            |

## 2026-07-27 marketing-ops 多项目通用化（C-20260727-133）

| 层级   | Case ID                      | 标题                                                    | 自动化路径                                  |
| ------ | ---------------------------- | ------------------------------------------------------- | ------------------------------------------- |
| L3     | TC-AUTO-PROJECT-133-01..03   | Project Profile 存储、schema 与文件系统安全             | plugin project-profile-store spec           |
| L3     | TC-AUTO-CONTRACT-133-01      | 七工具 contract v3 与 projectId 必填                    | public/plugin MCP contract specs            |
| L3/MCP | TC-AUTO-ISOLATION-133-01..02 | 项目级幂等/receipt 与跨项目 operation 拒绝              | plugin publish/receipt/operations specs     |
| L3     | TC-AUTO-TARGET-133-01        | URL、canonical 与渠道 profile policy                    | plugin project-policy spec                  |
| L3/MCP | TC-AUTO-GITHUB-133-01        | GitHub repository/tag/activation 项目上下文             | plugin GitHub channel/adapter/runtime specs |
| L3/MCP | TC-AUTO-DEV-133-01           | DEV canonical origins/tags 项目上下文                   | plugin DEV adapter/runtime specs            |
| L3     | TC-AUTO-MIGRATION-133-01     | v1 receipt 与旧 GitHub activation 兼容                  | plugin receipt/activation migration specs   |
| L3/CLI | TC-AUTO-CLI-133-01           | project add/list/show 与项目级 GitHub setup/status      | plugin CLI specs                            |
| L3     | TC-AUTO-BRIDGE-133-01        | Algorithm Visualizer projectId payload 与 contract 镜像 | `scripts/marketing/*contract*.spec.ts`      |
| build  | TC-AUTO-PLUGIN-133-01        | 通用插件包装、validator、stdio、coverage 与泄漏扫描     | plugin delivery gates                       |
...
| docs     | TC-DOC-AUTO-127-04             | C127 verified/100%，T1-T6 完成，下一步 content-studio | `docs/plans/20260711-c127-auto-distribution/test-cases.md` |
...
| L3       | TC-AUTO-CHANNEL-127-01..05     | 19 渠道、4 自动、12 Owner 辅助与 fail-closed gate     | `scripts/marketing/channels.spec.ts`                       |
...
| smoke    | TC-AUTO-BSKYSMOKE-127-01       | publish/read/幂等/delete 与远端清理已通过             | official API local smoke                                   |
| L3       | TC-AUTO-DEVAPI-127-00..06      | 固定 Forem v1、分页/资源边界、文章/评论与错误脱敏     | personal plugin `src/dev-api.spec.ts`                      |
| L3       | TC-AUTO-DEVADAPTER-127-01..07  | 英文 durable article、幂等、receipt 与失败关闭        | personal plugin `src/dev-article-adapter.spec.ts`          |
| L3       | TC-AUTO-DEVACT-127-01..04      | 0600 activation、身份、损坏、权限与版本严格拒绝       | personal plugin `src/dev-activation-store.spec.ts`         |
| L3       | TC-AUTO-DEVCHANNEL-127-01..07  | 隐藏 setup、Keychain、activation 与实时身份对拍       | personal plugin `src/dev-channel.spec.ts`                  |
...
| 层级  | Case ID                                | 标题                                             | 自动化路径                                              |
| ----- | -------------------------------------- | ------------------------------------------------ | ------------------------------------------------------- |
| L3    | TC-AUTO-MASTOAPI-127-01..07/02A        | 固定 v1 API、本地 acct 补全、状态/通知与错误脱敏 | personal plugin `src/adapters/mastodon-api.spec.ts`     |
| L3    | TC-AUTO-MASTOADAPTER-127-01..06        | 中英文 status、幂等、receipt、限制与安全删除     | personal plugin `src/adapters/mastodon-status.spec.ts`  |
| L3    | TC-AUTO-MASTOACT-127-01..04            | activation、身份/损坏/版本失败关闭               | personal plugin `src/mastodon-activation-store.spec.ts` |
| L3    | TC-AUTO-MASTODONCHANNEL-127-01..07/03A | 隐藏 setup、Keychain 顺序与实时身份对拍          | personal plugin `src/mastodon-channel.spec.ts`          |
| L3    | TC-AUTO-MASTOOBS-127-01..03            | lifetime 指标、untrusted 通知与 receipt 边界     | personal plugin `src/mastodon-observability.spec.ts`    |
| MCP   | TC-AUTO-MASTORUNTIME-127-01            | 请求级惰性注册与失去健康时失败关闭               | personal plugin `src/local-runtime.spec.ts`             |
| smoke | TC-AUTO-MASTOSMOKE-127-01              | 固定正文/UTM/幂等键与授权前零副作用              | `scripts/marketing/publish-payload.spec.ts`             |
| smoke | TC-AUTO-MASTOSMOKE-127-02              | matching 授权后的发布、读取、报告与安全清理      | local MCP real smoke                                    |

### 2026-07-28 调度、报告与反馈安全 T4

| 层级   | Case ID                     | 标题                                            | 自动化路径                                   |
| ------ | --------------------------- | ----------------------------------------------- | -------------------------------------------- |
| L3/MCP | TC-AUTO-SCHEDULE-127-01..04 | 三窗口计划、恢复与到期前零采集                  | plugin follow-up schedule + T4 runtime specs |
| L3/MCP | TC-AUTO-REPORT-127-01..04   | 标准指标、不可用值、归因与部分失败              | plugin campaign report + T4 runtime specs    |
| L3     | TC-AUTO-POLICY-127-01       | 0700/0600 project policy 与失败关闭             | plugin campaign-policy-store specs           |
| L3/MCP | TC-AUTO-FAQ-127-01..04      | FAQ 固定模板、升级边界与写前对拍                | plugin feedback-policy + T4 runtime specs    |
| L3     | TC-AUTO-GHREPLY-127-01..05  | GitHub Issue comment typed transport 与远端幂等 | plugin github-issue-reply-adapter spec       |
| L3/MCP | TC-AUTO-BUGROUTE-127-01..05 | Bug 双信号、最小化 Issue 与 receipt/marker 幂等 | plugin feedback-policy + T4 runtime specs    |

### 2026-07-28 Owner 辅助发布交接 T5

| 层级   | Case ID                     | 标题                                                      | 自动化路径                                 |
| ------ | --------------------------- | --------------------------------------------------------- | ------------------------------------------ |
| L3/MCP | TC-AUTO-ASSISTED-127-01..10 | 19/12 渠道边界、内容包、prepare/confirm、URL/receipt 安全 | public renderer/contract + plugin T5 specs |
...
| docs | TC-DOC-GROWTH-123-03 | 历史逐帖审批约束；已由 C127 supersede     | `docs/plans/20260710-c123-growth-execution/test-cases.md` |
...
共 **1193** 个用例。运行命令：`pnpm test:unit`
...
| TC-DATA-PATHS-01  | 四条路径 url 全有效无重复                                               | `src/data/paths.spec.ts`                    |
...
### 2-SAT 页 C-074（图算法第 8 页，新页，全模板 + 复用 GraphView·additive checkPair）

| 用例 ID         | 场景                                  | 文件                                         |
| --------------- | ------------------------------------- | -------------------------------------------- |
| TC-VIEW-2SAT-01 | 挂载渲染 Article + AlgorithmPlayer    | `src/views/Article/Algorithm/TwoSat.spec.ts` |
...
### 中国剩余定理页 C-087（数学与数论第 6 页，新页，纯复用 MatrixView 第 9 消费者）

| 用例 ID        | 场景                                      | 文件                                      |
| -------------- | ----------------------------------------- | ----------------------------------------- |
| TC-VIEW-CRT-01 | Article + AlgorithmPlayer                 | `src/views/Article/Algorithm/Crt.spec.ts` |
...
| TC-VIEW-PATHS-01       | 四张路径卡 + h1                                    | `src/views/Article/Paths.spec.ts`                 |
| TC-VIEW-PATHS-02       | 步骤链接 + 编号有序                                | `src/views/Article/Paths.spec.ts`                 |
| TC-VIEW-PATHS-03       | title 含大类名                                     | `src/views/Article/Paths.spec.ts`                 |
| TC-VIZ-SEARCH-08       | 面板空态学习路径快捷行（C-115）                    | `src/components/SearchPalette.spec.ts`            |
