...
- DEV 固定使用 Forem API v1 与 `api-key` 请求头；只允许单篇英文、无媒体、带项目 canonical 的 Markdown 文章。API key 仅经隐藏 TTY 写入 macOS Keychain，0600 activation 只保存公开 username/user ID；本人文章完整分页查询、确定性 marker 与本地 receipt 共同保证幂等。指标仅采集稳定文章响应中的 reactions/comments，评论正文一律标为 untrusted；当前官方作者接口没有真实删除文章能力，因此 `reply=false`、`delete=false`，不得把改回草稿描述为删除。
...
- DEV T3-D3-A 工程与零副作用 preflight 已完成；本机仍为 `not-configured / disabled`。Owner 只需在 DEV 创建专用 API key，并在本机隐藏向导完成一次性 setup；不得把 key 发到聊天、命令参数、环境变量或仓库。正式英文文章需对固定 campaign 另行明确授权，发布后会长期保留，不使用“临时测试后删除”的承诺。
