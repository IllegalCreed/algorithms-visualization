...
- [ ] 首次接入可通过 `setup` 向导完成，`status/doctor` 只显示账号别名、健康状态和下一步；正常 campaign 不要求 Owner 操作 CLI 或 JSON。
...
- T3 接入具体 adapter 时，Owner 只需在首次向导中确认微博、Bluesky、DEV、Mastodon 是否有账号；不要在聊天发送密码或 token。Codex 届时逐步引导，日常 campaign 不要求再次操作向导。
