...
- DEV 固定使用 Forem API v1 与 `api-key` 请求头；只允许单篇英文、无媒体、带项目 canonical 的 Markdown 文章。API key 仅经隐藏 TTY 写入 macOS Keychain，0600 activation 只保存公开 username/user ID；本人文章完整分页查询、确定性 marker 与本地 receipt 共同保证幂等。指标仅采集稳定文章响应中的 reactions/comments，评论正文一律标为 untrusted；当前官方作者接口没有真实删除文章能力，因此 `reply=false`、`delete=false`，不得把改回草稿描述为删除。2026-07-15 一次性 setup 与只读身份对拍已完成，status/doctor 为 ready/enabled，尚无 receipt/文章。
...
- DEV 当前尚无 receipt/文章。Owner 只需对固定 `marketing-ops-t3d3-smoke-127` 正式英文文章另行明确授权；发布后会长期保留，不使用“临时测试后删除”的承诺。
