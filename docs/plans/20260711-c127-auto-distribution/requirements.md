...
- 微博采用官方 `@weibo-ai/weibo-cli`；CLI 的平台 action 目录由登录账号和套餐动态返回，未完成 setup 前不得猜测 `statuses` 写 action。T3-D1-A 只实现固定进程边界、`doctor` 健康、只读 statuses 目录和注入式 fake adapter；production publish 必须等 Owner 完成官方 OAuth、个人开发者认证与 Free/试用 gate 后再冻结命令并显式启用。
...
- 微博 setup 只能调用官方浏览器/设备 OAuth；`marketing-ops` 不接受 `--token`、`auth token --export`、微博主密码、Cookie 或 token 环境变量。Free 若要求个人开发者认证或领取试用，由 Owner 在官方页面完成，不为通过 gate 购买套餐。
