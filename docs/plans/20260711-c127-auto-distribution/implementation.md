...
T0 调研和方案设计、T1 公开基础层、T2 MCP 安全运行时骨架均已完成。`scripts/marketing/` 现包含版本化 schema/严格 validator、规范化与 SHA-256 幂等键、15 渠道能力注册表和 runtime gate、站点事实快照及对拍、渠道 renderer、示例 campaign、`pnpm marketing:dry-run`，以及精确七工具的公开 MCP contract。CLI 只输出候选内容、UTM、选择/阻塞原因和 `sideEffects=[]`。

独立 personal plugin 已在本机 `/Users/zhangxu/plugins/marketing-ops` 建立并以本地提交 `a53f411` 固定。它通过 stdio 暴露精确七个高层 MCP 工具，提供一次性 `setup`、只读 `status/doctor`、macOS Keychain helper、每渠道独立 Profile、campaign 锁、0600 原子 receipt 存储与输出脱敏；凭据只经子进程 stdin/隐藏输入进入 Keychain，不进入 argv、env、JSON、日志或 MCP 输出。

当前仍没有真实平台 adapter、账号授权、已保存凭据、collector 或站外发布动作，不能表述为“全自动系统已经可用”。五渠道默认状态全部失败关闭；GitHub/微博/Mastodon 的真实授权与所有平台写入均从 T3 开始。首次接入时由 Codex 带 Owner 走一次向导，日常使用目标仍是只给自然语言 campaign。
...
- 主仓库 `pnpm verify` 通过：298 个 Vitest 文件 / 2124 个用例全绿，production base 预渲染并验证 190 页。
