...
- [ ] T3-D3-B：Owner 创建专用 DEV API key，并在隐藏 TTY 完成一次性只读身份 setup；不在聊天或仓库录入 secret。
- [ ] T3-D3-C：Owner 对 durable campaign 单独明确授权后执行 publish/read/幂等/反馈与报告 smoke；文章长期保留，不执行伪删除。
...
Bluesky 已完成隐藏 setup、身份对拍、安全删除和固定真实 smoke，临时记录已清理。DEV 已完成工程 adapter、collector 与公开 durable campaign preflight，但仍为 not-configured/disabled，Owner 尚未录入 API key，也未授权 DEV 文章写入。Mastodon 和完整 1h/48h/7d 调度仍未完成，不能表述为“全自动系统已经可用”。
