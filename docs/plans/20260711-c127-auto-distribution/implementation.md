...
- [ ] T3-D2-B3：matching campaign 明确授权后完成低风险 publish/read/delete smoke 与公开 URL 清理复查。
...
- 公开 smoke spec 与 preflight runtime 已固定；`pnpm marketing:dry-run` 输出唯一 `EXECUTION_NOT_APPROVED`、无 render issue、`sideEffects=[]`，本地 `marketing-ops-t3d2-smoke-127` receipt 数为 0。未调用 publish/delete 写接口。
...
- 2026-07-14：Bluesky setup 与安全删除完成：status/doctor ready/enabled，plugin `5d9aef1` 为 29/144 全绿；固定 smoke dry-run 只剩 `EXECUTION_NOT_APPROVED`，尚未发布帖子。
