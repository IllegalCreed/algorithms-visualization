...
- [ ] 获得 matching campaign 明确授权后执行唯一 create/read/delete/tag-cleanup 真实 smoke。
...
本机现有 GitHub CLI 账号 `IllegalCreed` 与目标仓库权限只读检查为 ready；扩展只读 smoke 已验证 traffic/Issues 可读，固定 smoke Release 与 tag ref 均不存在。`setup github` 尚未执行，activation 文件不存在、adapter 仍 disabled；未新增或读取聊天凭据，未调用 POST/PATCH/PUT/DELETE，未产生站外发布。下一步只能在 Owner 明确授权 `marketing-ops-t3c-smoke-127` 后执行一次 create/read/delete/tag-cleanup；其他渠道和完整 1h/48h/7d 调度仍未完成，不能表述为“全自动系统已经可用”。
