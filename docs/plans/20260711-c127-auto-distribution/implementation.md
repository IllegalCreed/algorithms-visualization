...
本机现有 GitHub CLI 账号 `IllegalCreed` 与目标仓库权限只读检查为 ready，但 `setup github` 尚未执行，activation 文件不存在，adapter 仍 disabled。T3-B 未新增或读取聊天凭据，未调用 Release POST/DELETE，未创建、删除 Release 或产生站外发布；collector 和其他渠道 adapter 也尚未实现，不能表述为“全自动系统已经可用”。下一步 T3-C 先固定 GitHub 低风险 smoke 证据与 Issue/collector contract，外部写入必须有对应 campaign 的明确授权。
