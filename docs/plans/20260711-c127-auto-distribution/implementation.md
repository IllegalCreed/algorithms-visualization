...
- [ ] T3-D2-B：Owner 确认或创建账号后完成一次性 setup；matching campaign 另行授权低风险 publish/read smoke。
...
- runtime 只为本次请求包含的 Bluesky package 惰性注册 adapter；默认 CLI 实测为 `not-configured / setup-required`。账号尚未接入，未读取或保存 Bluesky secret，零网络写入。
