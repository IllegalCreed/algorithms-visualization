...
| V6   | pre-commit | 提交时触发 lint-staged           | 自动 lint+format 暂存文件     | ✅ 首次提交时 lint-staged 自动格式化 docs 并 re-stage |
...
- V1–V5 即 CI 门禁链路，已按 `.github/workflows/deploy.yml` 顺序在本地完整验证通过，并于合并 main 后由 **GitHub Actions 实跑全绿**（run 27744663838，success）。
