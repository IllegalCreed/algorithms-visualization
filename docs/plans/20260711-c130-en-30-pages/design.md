...
- catalog 成为 30 组页面对的唯一结构事实源；29 个英文内容路由由显式静态 loader map 生成。
- Home、Menu、Search、Complexity、Paths、SEO registry 与语言切换均消费 catalog 派生结果。
- 27 个 adapter 拆到 `src/i18n/en/modules/`；共享 helper 先执行算法专属替换，再执行通用展示替换，保持步骤、轨道和 lineMap 不变。
- `pilot.ts` 仅作为迁移设想，所有消费者切换完成后直接删除，没有把兼容别名留成第二入口。
- prerender 与 verifier 不再硬编码 105/125 或 pilot slug，而是从页面和 manifest 动态发现并交叉校验当前集合。
...
- 2026-07-11：Owner 批准 C130；状态转 implementing，先完成 10/105 等价迁移再分批扩容。
- 2026-07-11：目标结构完成，最终实现移除 pilot 兼容层并把构建页数校验改为动态发现；等待双轨发布。
- 2026-07-11：Pages/selfhost 均上线 125 页并通过代表 URL、canonical、alternate、正文语言与 sitemap 抽查；状态转 verified。
- 2026-07-11：C131 复用本设计继续完成剩余 65 页；C130 的 30 页验收事实不变。
- 2026-07-11：C131 全量实现与双轨发布通过；本设计的扩展路径验证完成。
