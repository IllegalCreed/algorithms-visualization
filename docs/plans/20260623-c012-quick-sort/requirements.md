...
- 2026-06-23（修订）：据用户反馈调整 `StackView` 视觉——栈帧从「按区间下标可变宽 + 缩进定位」改为**固定等宽居中**；记法从 `[lo,hi]` 改为 `a[lo..hi]`（更明确是「数组的一段」而非「一条数据」）；用 `<TransitionGroup>` + 稳定 key 补**入栈/出栈动画**（修复旧实现 `:key` 用下标 + 无过渡导致出栈时帧瞬间消失）。需求与执行点不变，仅 `StackView` 视觉实现 + 测试调整（`TC-VIZ-STACKVIEW` 5→6）。
