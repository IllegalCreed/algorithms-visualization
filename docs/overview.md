...
| 测试     | Vitest L3/L4：280 个测试文件、2033 个用例在 2026-07-10 本地全绿；Playwright L5：103 个文件、110 个用例全绿；coverage 与双 base 构建门禁通过 |
| 当前阶段 | M9-M12 全清单完成；C-20260710-124 SEO/GEO 已完成双轨上线验证；当前阶段 C125 分析归因，随后是 `/en` 试点、半自动分发和发布复盘               |
| 增长现状 | 95 页 route head/canonical/JSON-LD、构建后静态 HTML、同源 sitemap/llms 与 crawler 策略已落地；分析、站点多语言、分发自动化尚未实现          |
