...
| P1     | 低风险维护修复       | ongoing  | C137/C138 已完成播放器同屏布局与阴影裁剪修复；继续处理不改变算法语义的小问题：可访问性、导航语义、搜索召回、文档事实、测试防回归 |
