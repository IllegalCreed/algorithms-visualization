...
当前不继续铺中文算法页或第三语言。中英文已各有 95 个索引页，共 190 个静态入口；C127 为 87%。微博零费用 API 发布路径已关闭；Bluesky 已完成一次性 setup、安全删除与 Owner 授权的固定 smoke，receipt 和远端 record 均已清理，下一步为 T3-D3 DEV adapter。第三方统计继续暂缓。
...
| 增长基线 | 95 中文 + 95 英文及 95 组 hreflang 已双轨上线；GitHub/Bluesky ready/enabled 且真实 smoke 已清理；微博 API disabled                             |
...
| P1     | 宣传自动化           | ongoing  | C127 87%；Bluesky setup、安全删除与真实 smoke 已完成并清理；下一步 T3-D3 DEV adapter                                      |
