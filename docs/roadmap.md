...
当前不继续铺中文算法页或第三语言。中英文已各有 95 个索引页，共 190 个静态入口；C127 保持 79%。微博认证已通过，但官方 Free 仅为 7 天只读试用且写入额度为 0，零费用 API 发布路径已关闭；当前转向 Bluesky，第三方统计继续暂缓。
...
| 增长基线 | 95 中文 + 95 英文及 95 组 hreflang 已双轨上线；GitHub ready/enabled；微博官方 CLI 无写 health 已落地，production adapter 仍 disabled           |
...
| P1     | 宣传自动化           | ongoing  | C127 79%；微博认证已通过，Free 为 7 天只读/零写额度，API 发布 disabled；下一步 Bluesky                                    |
