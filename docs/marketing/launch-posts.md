...
> Automation status: `docs/marketing/channel-automation-audit.md`
> 说明：这是 C-118 留下的掘金、V2EX、B站首发初稿，不代表三个渠道都支持自动发布。C127 当前结论为掘金人工、V2EX 人工发布后可自动监测、B站需开放平台主体/应用/UP 授权；实际执行以渠道审计为准。
...
1. C127 先完成首批官方 adapter 与至少一个非 GitHub 真实 smoke，再进入 C128。
2. 掘金/V2EX 使用本文件草稿时由 Owner 在官方 UI 发布；返回 V2EX URL 后由 collector 自动监测。B站仅在开放平台授权完成后自动发布。
3. 每个真实发布分别记录 48h/7d 数据，不再预设同日发布或固定上午 10 点一定更优；根据证据选择后续渠道和时间。
...
- [ ] 提供录屏动图三段（快排自定义输入 / 测验答题 / ρ 现形）；C128 发帖渠道与方式按能力审计执行
...
- 2026-07-11：C-127 渠道审计确认三个历史草稿的自动化能力不同；保留文案资产，执行方式改由渠道审计和 C127 能力 gate 决定。
