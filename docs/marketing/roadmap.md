...
3. route head、尾斜杠 canonical、JSON-LD、按 catalog 生成的 sitemap/llms、95 组双向 hreflang 与 crawler 策略已落地；第三方分析已撤销。C127 为 85%；微博 Free 零写额度，Bluesky 已 ready/enabled 且固定 smoke 只等待 matching campaign 授权。
...
| C127 | 微博 API disabled；Bluesky setup、安全删除与 dry-run 完成，下一步固定 campaign smoke | in-progress |
...
- 2026-07-14：Bluesky setup 后为 ready/enabled；plugin `5d9aef1` 补齐安全删除，固定 smoke dry-run 仅缺 `EXECUTION_NOT_APPROVED`，尚未发布帖子。
