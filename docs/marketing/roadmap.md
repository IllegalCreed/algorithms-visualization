...
| -------- | ------------------------ | -------------------------------------------------------------------------------------------------------- | -------------------------------------- | ---------------------------------------- |
| 国内中文 | 学生、求职者、中文开发者 | GitHub API 自动候选；微博/V2EX/掘金/知乎/简书/B站走 Owner 辅助发布；公众号暂不生成包                     | C127 + content-studio + matching 授权  | 获取种子用户、收集产品反馈、比较渠道效率 |
| 海外英文 | 英文学习者与开发者       | Bluesky/DEV/Mastodon/GitHub 为 API 自动候选；X/Facebook/YouTube/HN/PH 走 Owner 辅助发布；Reddit 条件接入 | C127 + content-studio；逐渠道授权/审核 | 验证英文需求、自然搜索与海外渠道         |
...
flowchart LR
  ...
  C134 --> C127B["C127 T4-T6 收尾"]
  C127B --> STUDIO["content-studio 内容与视频生产"]
  STUDIO --> C128["C128 发布与复盘"]
```
...
- 普通内容生成提示词不授予站外写权限；只有 Owner 对匹配 campaign 明确授权的指令才允许自动渠道执行写入，owner-assisted 渠道还由 Owner 控制官方 UI 最终发布。
...
- 2026-07-11：C127 T3-A 以公开仓库 `98f8deb` 与本地插件 `ba6d4c3` 完成 MCP v2、共享 adapter contract、GitHub Release typed fake 与 dispatch；默认 server 仍无 live client 或真实写入，下一步 T3-B。
- 2026-07-11：C127 T3-B 以本地插件 `98a9dfc` 完成固定 GitHub CLI、只读账号/仓库健康检查、0600 activation 与惰性 runtime；activation 仍缺失、adapter disabled、真实写入为零，下一步 T3-C。
- 2026-07-11：C127 T3-C 以插件 `60feaff` 完成 collector、MCP 查询撤回与 Release/tag 安全清理；Owner 授权的临时 Release `352517542` 随后完成 create/read/report/delete，三侧清理复查通过，当前 76%，下一步 T3-D。
- 2026-07-11：C127 T3-D1-A 以插件 `3858b56` 完成官方微博 CLI 固定进程、零费用 health gate、只读 statuses 目录与注入式文本 adapter contract；25 文件/111 用例、coverage、verify 和隔离 doctor 全绿，当前 79%，production adapter 保持 disabled。
- 2026-07-12：C127 T3-D1-B 完成微博固定版本安装与 device OAuth；插件 `088229d` 兼容真实 null subscription。个人开发者认证审核中，未读 catalog、未启用 adapter、零写入。
- 2026-07-14：微博个人认证已通过；官方 Free 复核为 0 元/7 天、仅本人数据、5 读/小时、0 写/小时。未领取试用、未读 catalog、零写入；plugin `263fd3f` 禁止将 Free 误报为可发布。
- 2026-07-14：Bluesky setup 后为 ready/enabled；plugin `5d9aef1` 补齐安全删除。Owner 授权的固定 smoke 随后完成 publish/read/同回执复放/delete/重复 delete，receipt deleted、远端 record 不存在，C127 转 87%，下一步 DEV。
