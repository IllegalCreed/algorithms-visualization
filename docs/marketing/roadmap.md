...
3. route head、尾斜杠 canonical、JSON-LD、按 catalog 生成的 sitemap/llms、95 组双向 hreflang 与 crawler 策略已落地；第三方行为分析已撤销。C127 为 94%；微博 API disabled，GitHub/Bluesky/DEV/Mastodon 真实闭环完成，DEV 正式文章长期公开但后续写入需重新授权；Mastodon 临时状态已删除且保持 ready/enabled，下一步进入 T4。
...
| -------- | ------------------------ | ---------------------------------------------------------------------------- | ------------------------------------- | ---------------------------------------- |
| 国内中文 | 学生、求职者、中文开发者 | GitHub 自动；微博/V2EX/掘金保留人工；微博 RPA 需后续独立评审；B站/公众号禁用 | C127 adapter + 免费个人账号授权       | 获取种子用户、收集产品反馈、比较渠道效率 |
