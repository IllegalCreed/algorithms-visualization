...
3. route head、尾斜杠 canonical、JSON-LD、按 catalog 生成的 sitemap/llms、95 组双向 hreflang 与 crawler 策略已落地；第三方分析已撤销。C127 保持 79%，微博个人认证已通过，但 Free 为 7 天只读试用且写入额度为 0，零费用 API 发布不再进入实施。
...
| C127 | 微博认证已通过，Free 为 7 天只读/零写额度，API 发布 disabled；下一步接入 Bluesky | in-progress |
