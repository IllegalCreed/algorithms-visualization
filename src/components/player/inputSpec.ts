...
export function parseInputArray(text: string, spec: InputSpec): ParseResult {
  const parts = text
    .split(/[,，\s]+/)
    .map((s) => s.trim())
    .filter((s) => s.length > 0);
  if (parts.length === 0) return { ok: false, error: '请输入数字' };

  const value: number[] = [];
  for (const p of parts) {
    const n = Number(p);
    if (Number.isNaN(n)) return { ok: false, error: `「${p}」不是数字` };
    if (!Number.isInteger(n)) return { ok: false, error: `「${p}」不是整数` };
    value.push(n);
  }
  if (value.length < spec.lenMin || value.length > spec.lenMax) {
    return {
      ok: false,
      error: `个数需在 ${spec.lenMin} ~ ${spec.lenMax} 之间（当前 ${value.length} 个）`,
    };
  }
  for (const n of value) {
    if (n < spec.valMin || n > spec.valMax) {
      return { ok: false, error: `数值需在 ${spec.valMin} ~ ${spec.valMax} 之间（「${n}」越界）` };
    }
  }
